...
- 继续沿用 Electron 主进程 + 静态渲染页面，不引入新框架或打包工具。
- 新增独立无边框悬浮按钮窗口和悬浮面板窗口，使用 `screen.getPrimaryDisplay().workArea` 定位到屏幕右下可用区域。
- 悬浮按钮窗口为 56x56；打开面板时显示 360x430 面板窗口，并通过 IPC 通知渲染层切换按钮/面板。
- `Ctrl+Shift+V` 改为打开悬浮面板；托盘菜单提供打开主窗口、打开悬浮面板、显示/隐藏悬浮按钮、退出。
- 设置结构新增 `floatingButtonVisible`，旧设置文件缺少该字段时默认归一化为 `true`。
- 历史记录结构保持不变，悬浮面板复用现有复制回剪贴板能力。
